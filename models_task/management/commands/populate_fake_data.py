...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--projects", type=int, default=5, help="Number of projects to create"
        )
        parser.add_argument(
            "--tasks", type=int, default=20, help="Number of tasks to create"
        )
        parser.add_argument(
            "--documents", type=int, default=15, help="Number of documents to create"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(num_projects):
            try:
                ...
                end_date = fake.date_between(
                    start_date=start_date, end_date=start_date + timedelta(days=180)
                )
                ...
                team_members = random.sample(
                    users, random.randint(2, min(5, len(users)))
                )
                ...
                self.stdout.write(
                    self.style.WARNING(f"Error creating project: {str(e)}")
                )
                continue
        ...
        for i in range(num_tasks):
            try:
                ...
                task = Task.objects.create(
                    ...
                    project=project,
                    assignee=(
                        random.choice(possible_assignees)
        ...
        for i in range(num_documents):
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Error creating document: {str(e)}")
                )
                continue
        ...
        for i in range(num_comments):
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Error creating comment: {str(e)}")
                )
                continue
